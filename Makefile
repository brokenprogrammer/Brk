...
LLVMCONFIG=$(llvm-config --cxxflags --ldflags --system-libs --libs core)

debug:
	$(CC) $(LLVMCONFIG) src/main.cpp src/globals.cpp src/lexer.cpp src/parser.cpp src/expression.cpp $(LLVMFLAGS) $(DISABLED_WARNINGS) $(CFLAGS) -g $(LDFLAGS) -o brk.exe

release:
	$(CC) $(LLVMCONFIG) src/main.cpp $(DISABLED_WARNINGS) $(CFLAGS) -O3 -march=native $(LDFLAGS) -o brk.exe
